...
const input = readAsLines(`${__dirname}/test-input.txt`);

const histories = input.map((line) => line.match(/(-?\d+)/g)!.map(Number)).map(reverse);
const prevValues = histories.map((history) => {
...
const sum = prevValues.reduce((sum, curr) => sum + curr, 0);
console.log(reverse([...prevValues]));
console.log(sum);
