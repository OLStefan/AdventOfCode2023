...
const loopGrid = grid.map((row, i) =>
	row.map((_, j) => (loop!.some((coord) => i === coord[0] && j === coord[1]) ? grid[i]![j]! : '.')),
);

let inside = false;
let lastCorner: Pipe = '.';
let tiles = 0;
const filledLoop = grid.map((row, i) =>
	row.map((_, j) => {
		const currTile = loopGrid[i]![j]!;
		switch (currTile) {
			case '.':
				if (inside) {
					tiles++;
